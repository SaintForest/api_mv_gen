...
def main(argv=None):
    ...
    if (1 < len(args["description"]) < 1000):
        print(f"Description argument has words and characters in range")

    if any(not c.isalnum() for c in args["description"].split()) == False:
        print(f"Title and description have only alphanumeric characters")

    if os.path.exists("model-new.pt") == False:
        print("Weights file is not present, downloading")
        gdd.download_file_from_google_drive(
            file_id="1lMjuBZ161egqHAnBtwNDHzv5hkzh4G26", dest_path="./model-new.pt"
        )
    ...
    encoded = tokenizer.encode_plus(
        args["description"], max_length=100, pad_to_max_length=True
    )
    ...
    print("\n \n \n \n")
    print({"description": args["description"], "genre": genres})
    return(genres)
